...
from plates import plate_slots                     # noqa: E402
...
BOXY = {"bed_long", "shelf", "planter_big", "shelf_two", "bench"}

# 판 있는 가구. 자리는 윗판 위에서 계산합니다(tools/plates.py).
#
# 선반은 선반일 뿐입니다 - 식물이 그려져 들어온 조각은 쓰지 않습니다.
# 판 위에 물건이 붙어 있으면 우리가 올릴 자리가 없습니다.
PLATE = {"shelf", "shelf_two", "bench"}
...
SMALL = ["sprout", "small"]            # 판 위에 올라갈 수 있는 작은 것들

SLOT_GRADE = {
    "pot_sprout": [["sprout"]], "pot_small": [["small"]],
    "pot_medium": [["medium"]], "pot_large": [["large"]],
    "pot_xlarge": [["xlarge"]],
    # 두 칸짜리 식물 하나가 아니라 한 칸짜리 두 그루
    "bed_long": [["small"], ["small"]],
    "planter_big": [["xlarge"]],
    # 자리가 없으면 가구입니다. 선반은 빈 그림을 아직 못 받았고, 아래 둘은
    # 화분과 식물이 그려져 들어와 장식으로만 씁니다.
    # 판 위에는 작은 것만. 자리마다 들어갈 수 있는 등급이 여럿입니다.
    "shelf": [SMALL, SMALL], "shelf_two": [SMALL, SMALL],
    "bench": [SMALL, SMALL],
}
...
def main(out="../app-kit/assets"):
    ...
    for pid, p in pieces.items():
        ...
        if pid in PLATE:
            # 판 위의 자리는 조각 상자에서 계산합니다. 시트가 바뀌어도
            # 같은 방법이 그대로 듣습니다. 기준은 **밑면 한가운데**입니다.
            p["anchor"] = plate_slots(art, cells, grid, to_stage, p["foot"][0])
        cat["pots"][pid] = {
            "w": art.width, "h": art.height, "cells": cells,
            "foot": {"x": round(fx), "y": round(fy)},
            "slots": [{"x": round(x), "y": round(y), "grades": g,
